...
for index,pth in enumerate(config.lms):
  ...
  if index>0:
    pth='lm_'+str(index-1)+'.proc'
  os.system(config.kenlm+'/lmplz -o '+str(config.lm_order)+' --discount_fallback < '+pth+' 1> '+pth+'.arpa 2>> '+config.working_dir+'/train.log')
  os.system(config.kenlm+'/build_binary '+pth+'.arpa '+pth+'.blm >> '+config.working_dir+'/train.log 2>&1')

sys.stdout.write('Building the untuned system\n')
sys.stdout.flush()
os.system(config.moses_scripts+'/training/train-model.perl -root-dir '+config.working_dir+' -corpus '+config.working_dir+'/train -f orig -e norm -alignment grow-diag-final-and -reordering msd-bidirectional-fe -lm 0:'+str(config.lm_order)+':'+config.working_dir+'/train.norm.blm:8 -cores '+str(config.num_cores)+' --mgiza -mgiza-cpus '+str(config.num_cores)+' -external-bin-dir '+config.mgiza+' >> '+config.working_dir+'/train.log 2>&1')
...
sys.stdout.flush()
os.system(config.moses_scripts+'training/mert-moses.pl dev.orig dev.norm '+config.moses+'/moses '+config.working_dir+'/model/moses.mod.ini --mertdir '+config.moses+' --working-dir '+config.working_dir+'/mert-work/ --mertargs="--sctype WER" --decoder-flags="-threads '+str(config.num_cores)+'" >> train.log 2>&1')
